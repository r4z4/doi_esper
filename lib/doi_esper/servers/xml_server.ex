defmodule DoiEsper.Servers.XmlServer do
  use GenServer
  ...
  defp publish(list) do
    Phoenix.PubSub.broadcast(
      DoiEsper.PubSub,
      @xml_parse,
      %{topic: @xml_parse, payload: %{status: :complete, data: list}}
    )
  end

  @impl true
  def handle_info(:xml_parse, state) do
    IO.puts "Handle XML Parse"
    # Call NIF
    file_path = "./priv/static/files/housing.xml"
    parsed_list = DoiEsper.XmlParse.parse_file(file_path)
    # svg = generate_svg()
    IO.inspect(parsed_list)
    publish(parsed_list)
